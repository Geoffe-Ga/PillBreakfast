...
**Component**: `Shared/` SwiftData schema + sync API

**Documentation**: Overview of the schema and what each model represents, the import path from
HealthKit (iPhone only, read-only — link SPEC §3), quick-start examples for fetching the regimen and
logging a `DoseEvent`, complete API reference for the sync service, the version policy for
WatchConnectivity payloads, migration notes for schema changes.
