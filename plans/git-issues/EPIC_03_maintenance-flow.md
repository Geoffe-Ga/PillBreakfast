...
- [ ] #NNN — Skeleton: iPhone Regimen tab list view + watch root "Right Now" view with stub data wired to the SwiftData store (EPIC_03_ISSUE_01).
- [ ] #NNN — iPhone add/edit/archive form for a maintenance medication, persisted to SwiftData (EPIC_03_ISSUE_02).
- [ ] #NNN — Watch tap-through queue with single-tap Mark Taken writing `DoseEvent`s (EPIC_03_ISSUE_03).
- [ ] #NNN — Local notification scheduler on the watch with full-rebuild semantics on regimen change (EPIC_03_ISSUE_04).
- [ ] #NNN — Reverse sync of `DoseEvent`s from watch to iPhone via `WCSession.transferFile` (EPIC_03_ISSUE_05).
- [ ] #NNN — Pending-queue selection logic with timezone and "already taken today" edge cases + unit tests (EPIC_03_ISSUE_06).
