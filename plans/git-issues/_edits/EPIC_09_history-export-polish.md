...
- [ ] #53 — Skeleton: Add the History tab with a placeholder heatmap reading from SwiftData, wired but rendering stub data (EPIC_09_ISSUE_01).
- [ ] #54 — Implement the 30-day calendar heatmap and per-day drill-down with status icons and PRN running totals (EPIC_09_ISSUE_02).
- [ ] #55 — Filter-by-medication control on the heatmap and drill-down (EPIC_09_ISSUE_03).
- [ ] #56 — `PDFKit` exporter that reads denormalized `DoseEvent.ingredientAmounts` and produces a paginated 30-day report with the seeded-ingredient disclaimer footer (EPIC_09_ISSUE_04).
- [ ] #57 — Share sheet integration via SwiftUI `ShareLink` targeting Mail / Messages / Files (EPIC_09_ISSUE_05).
- [ ] #58 — Empty states + error-handling pass (WatchConnectivity unreachable, HealthKit denied, SwiftData write failure) (EPIC_09_ISSUE_06).
- [ ] #59 — VoiceOver audit across iPhone and watch interactive elements with explicit accessibility labels and traits (EPIC_09_ISSUE_07).
