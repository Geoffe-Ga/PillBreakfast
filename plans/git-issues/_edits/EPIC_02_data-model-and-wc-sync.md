...
- [ ] #16 — Skeleton: Add `Shared/Models/` with empty `@Model` classes wired into the container (EPIC_02_ISSUE_01).
- [ ] #17 — Implement the schema body from SPEC §5.2 with `Sendable` review and unit tests (EPIC_02_ISSUE_02).
- [ ] #18 — Add the seeded ingredient library with idempotent first-launch insertion (EPIC_02_ISSUE_03).
- [ ] #19 — Add `RegimenSnapshot` DTO + Codable round-trip tests (EPIC_02_ISSUE_04).
- [ ] #20 — Push the hardcoded stub medication from iPhone to watch via `updateApplicationContext` and render its name (EPIC_02_ISSUE_05).
