...
- [ ] #NNN — Skeleton: Add the watch widget extension target with a single complication family rendering a stub pending count (EPIC_08_ISSUE_01).
- [ ] #NNN — Implement all three complication families (circular, corner, inline) with deep-link to the tap-through queue (EPIC_08_ISSUE_02).
- [ ] #NNN — Smart Stack widget that surfaces 15 min before a scheduled dose with Liquid Glass styling (EPIC_08_ISSUE_03).
- [ ] #NNN — `LogNextDoseIntent: AppIntent` wired to single-tap widget logging, with the high-risk "open to confirm" fallback (EPIC_08_ISSUE_04).
- [ ] #NNN — Background refresh on both targets calling `WidgetCenter.shared.reloadAllTimelines()` after dose writes (EPIC_08_ISSUE_05).
