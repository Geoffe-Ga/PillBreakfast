...
- [ ] #NNN — Skeleton: Register the `SNOOZE_UNTIL_TIME` action and add a stub `SnoozeView` that closes without rescheduling (EPIC_06_ISSUE_01).
- [ ] #NNN — Implement the cancel-then-schedule round-trip in `Shared/Notifications/` with unit tests, including post-midnight rollover (EPIC_06_ISSUE_02).
- [ ] #NNN — Wire `SnoozeView` (`DatePicker(.hourAndMinute)` + Done) to the snooze scheduler with Liquid Glass styling (EPIC_06_ISSUE_03).
- [ ] #NNN — Add `DoseEvent.snoozeCount` with additive SwiftData migration and the fourth-snooze soft warning interstitial (EPIC_06_ISSUE_04).
- [ ] #NNN — iPhone Settings entry for default snooze picker position (SPEC §6.3) (EPIC_06_ISSUE_05).
