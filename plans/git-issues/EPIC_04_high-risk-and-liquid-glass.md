...
- [ ] #NNN — Skeleton: Add a `LiquidGlassTheme` design-system module in `Shared/DesignSystem/` with token APIs (colors, materials, typography) and apply it to the watch root view as a smoke test (EPIC_04_ISSUE_01).
- [ ] #NNN — Press-and-hold gesture with progress ring for high-risk meds on the watch tap-through screen (EPIC_04_ISSUE_02).
- [ ] #NNN — Apply `.glassEffect()` and Material backgrounds to the watch tap-through queue, success state, and iPhone Regimen tab (EPIC_04_ISSUE_03).
- [ ] #NNN — Settings entry on iPhone for press-and-hold duration, persisted and synced to the watch (EPIC_04_ISSUE_04).
- [ ] #NNN — Snapshot tests for high-risk vs. non-high-risk tap-through screens, plus a manual visual-review checklist in the PR template (EPIC_04_ISSUE_05).
